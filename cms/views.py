...
@csrf_exempt
def editPage(request, text):
    template = get_template('edit_form.html')
    if request.user.is_authenticated():
        message = "Logueado como " + str(request.user) + "."
        link = "<a href=http://localhost:8000/logout/>Logout</a><br><br>"
    else:
        message = "Usuario no logueado."
        link = "<a href=http://localhost:8000/authenticate/>Login</a><br><br>"
    if request.method == "GET":
        if request.user.is_authenticated():
            try:
                object = Pages.objects.get(name = text)
                return HttpResponse(template.render(Context({'message': message, 'link': link, 'resource': text, 'page': object.page})))
            except Pages.DoesNotExist:
                return HttpResponse(message + "No hay una página para " + text)
        else:
            return HttpResponse(message + link + "Para editar una página primero debes loguearte.")
    elif request.method == "POST":
        if request.user.is_authenticated():
            object = Pages.objects.get(name = text)
            object.page = request.POST['newPage']
            object.save()
            template = get_template('edit_form.html')
            return HttpResponse(template.render(Context({'message': message, 'link': link, 'resource': text, 'page': object.page})))
        else:
